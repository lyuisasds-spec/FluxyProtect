...
const app = express();  // <--- ESTO DEBE ESTAR ANTES DE CUALQUIER RUTA
...
const BOT_INTERNAL_URL = process.env.BOT_INTERNAL_URL || 'http://localhost:10000';
...
app.post('/api/create-script', requireAuth, async (req, res) => {
    try {
        ...
        const scriptKey = Math.random().toString(36).substring(2, 10).toUpperCase() + 
...
// ============================================
// RUTA PARA ENVIAR PANEL A DISCORD (CORREGIDA)
// ============================================
app.post('/api/send-panel/:id', requireAuth, async (req, res) => {
    try {
        ...
        try {
            const response = await fetch(`${BOT_INTERNAL_URL}/api/bot/send-panel`, {
                method: 'POST',
